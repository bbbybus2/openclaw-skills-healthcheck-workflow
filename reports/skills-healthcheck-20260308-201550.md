...
- Timestamp: 2026-03-08T20:15:50+08:00
- Host: VM-0-4-ubuntu
- Runner: ubuntu
...
- ✅ **ffmpeg installed (video-frames)**
  ...
  - Result: PASS
- ✅ **summarize model call (Gemini)**
  - Command: `[[ -n "${GEMINI_API_KEY:-}" ]] && echo "OpenClaw summarize smoke test" > /tmp/summarize-smoke.txt && GEMINI_API_KEY="${GEMINI_API_KEY:-}" summarize /tmp/summarize-smoke.txt --model google/gemini-3.1-pro-preview --length short >/dev/null`
  - Result: PASS
- ✅ **gog account listing**
